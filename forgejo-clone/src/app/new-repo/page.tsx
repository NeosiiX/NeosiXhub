    ...
    <div className="max-w-2xl mx-auto py-10 px-4 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="border-border/60">
          <CardHeader>
            <CardTitle className="text-base">Informations du dépôt</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name">Nom du dépôt *</Label>
              <Input id="name" placeholder="mon-projet" value={form.name} onChange={update("name")}
                pattern="[a-zA-Z0-9._-]+" title="Lettres, chiffres, ., _ et - uniquement" required />
              <p className="text-xs text-muted-foreground">Les bons noms sont courts et mémorables.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="branch" value={form.defaultBranch} onChange={update("defaultBranch")} placeholder="main" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="license">Licence <span className="text-muted-foreground">(optionnel)</span></Label>
              <select
                id="license"
                value={form.license}
        ...
        <Card className="border-border/60">
          <CardHeader>
            ...
            <CardDescription>Choisissez qui peut voir ce dépôt.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {[
              ...
              <label key={label}
                className={`flex items-start gap-3 p-4 rounded-lg border cursor-pointer transition-colors ${form.isPrivate === value ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
        ...
        <Card className="border-border/60">
          <CardContent className="pt-6">
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" className="rounded" checked={form.initReadme}
                onChange={(e) => setForm({ ...form, initReadme: e.target.checked })} />
              <div>
                <p className="text-sm font-medium">Initialiser ce dépôt avec un README</p>
                <p className="text-xs text-muted-foreground">Cela permet de cloner le dépôt immédiatement.</p>
              </div>
            </label>
          </CardContent>
        </Card>
